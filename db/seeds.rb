...
CSV.foreach(csv_file_path, headers: true).with_index(1) do |row, index|
  ...
  kanji.meaning_list = row['meanings']
  kanji.on_reading_list = row['on'].split('|') unless row['on'].nil?
  kanji.kun_reading_list = row['kun'].split('|') unless row['kun'].nil?

  if kanji.save
    puts "Kanji #{kanji.character} created successfully."
  else
    puts "Error creating Kanji #{kanji.character}: #{kanji.errors.full_messages.join(", ")}"
  end
end
