...
const extractText = async () => {
  ...
  const recognizedKanji = stripKanji(ret.data.text);
...
canvas.addEventListener('mousemove',  (event) => { updateMousePos(event); dragCanvas(); });
canvas.addEventListener('mouseup',    (event) => { updateMousePos(event); endTouchCanvas(); });
canvas.addEventListener('touchstart', (event) => { updateTouchPos(event, newTouch=true); startTouchCanvas(event); });
canvas.addEventListener('touchmove',  (event) => { updateTouchPos(event); dragCanvas(event); });
canvas.addEventListener('touchend',   (event) => { updateTouchPos(event); endTouchCanvas(event); });
// MIGRATED
